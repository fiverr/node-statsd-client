 ...
 */
module.exports = function sender({host, port, protocol, protocol_version, errorHandler, timeout} = {}) {
    if (protocol) {
    ...
    if ((/tcp/i).test(protocol)) {
        return sendTCP(port, host, errorHandler, timeout);
    }
    ...
    return sendUDP(port, host, sockettype, errorHandler, timeout);
};
